...
while True: #Loop to get country code for holidays if user would like to have it
    holidays_included = None
    print("Would you like to have holidays marked on calendar per provided country? y/n")
    response = input("> ").lower()

    if response.startswith("y"):
        print("Enter the country code for which you would get holidays mark in calendar.")
        print("Country code should be provided as 2 letters code i.e US, PL etc.")
        response = input("> ")

        if response not in COUNTRY:
            print("Sorry, such country is not applicable - please try again.")
            continue
        elif response in COUNTRY:
            holidays = holidays.country_holidays(response)
            holidays_included = True
            global response_country
            response_country = response
            break
    else:
...
def getCalendarFor(year, month):
    ...
    #The datetime module handles all the complicated calendar stuff for us here.
        ...
        for i in range(7):
            ...
            if holidays_included:
                if currentDate in holidays:
                    dayNumberRow += "|" + dayNumberLabel + " HOLIDAY"
                    currentDate += datetime.timedelta(days=1)
                else:
...
if holidays_included:
    calendarFilename = "calendar_{}_{}_{}.txt".format(year, month, response_country)
else:
