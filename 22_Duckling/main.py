...
class Duckling:
    def __init__(self):
        ...
        if self.body == CHUBBY:
            #Chubby duckling can only have beady eyes
            self.eyes = BEADY
        else:
    ...
    def getBodyStr(self):
        ...
        if self.body == CHUBBY:
            #Get an extra space so chubby ducklings are
            #the same width as very chubby ducklings
            bodyStr += ' '
    ...
    def getNextBodyPart(self):
        """Calls the appropriate display method for the
        next body part that needs to be displayed.
        Sets partToDisplayNext to None when finished"""
