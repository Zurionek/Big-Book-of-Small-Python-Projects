...
DICE_HEIGHT = 5
CANVAS_WIDTH = 79
CANVAS_HEIGHT = 24 - 3 #-3 to enter the sum at the bottom
...
You have {} seconds to answer as many as possible.
You get {} points for each correct answer and lose {} point for each incorrect.
...
while time.time() < startTime + QUIZ_DURATION: #Main game loop
    ...
    for i in range(len(diceFaces)):
        while True:
            ...
            #Check if this die overlaps with previous one
            overlaps = False
            ...
            if not overlaps:
                #It does not overlap, so we can put it here:
