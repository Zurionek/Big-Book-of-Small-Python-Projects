"""Etching Drawer, by Al Sweigart
An art program that draws a continuous line around the
screen using the WASD keys. Inspired by Etch a Sketch toys.
...
#We can't print to the last column on the Windows without it
...
"""The keys for canvas will be (x,y) integer tuples for
the coordinate, and the value is a set of letters W, A, S, D
that tell what kind of line should be drawn
"""
canvas = {}
cursorX = 0
cursorY = 0
...
def getCanvasString(canvasData, cx, cy):
    """Returns a multiline string of the line draw in canvasData."""
    canvasStr = ''

    """canvasData is a dictionary with (x,y) tuple keys
    and values that are sets of 'W', 'A', 'S' and/or
    'D' strings to show which directions the lines are drawn
    at each xy point.
    """
...
while True: #Main program loop
    ...
    if response == "QUIT":
        ...
        print("draws a line behind it as it moves. For example, ddd")
    ...
    for command in response:
        ...
        if command == "W" and cursorY > 0:
            ...
            #If the cursor does not move because it would have moved off
        ...
        #which will be used to add new command draw in next iteration
